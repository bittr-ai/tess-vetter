...
class target:
    ...
    def remove_star(self, drop_stars: np.ndarray):
        ...
        Args:
            drop_stars (numpy array): Array of (str) TIC IDs
                for stars to drop.
        """
    ...
    def update_star(self, ID: int, param: str, value: float):
        ...
        """
        idx = self.stars[str(ID) == self.stars.ID].index
    ...
    def calc_probs(self, time: np.ndarray, flux_0: np.ndarray,
        ...
        num_external_lcs = len(external_lc_files) if external_lc_files else 0
        ...
        best_u1_p = [np.zeros(N_scenarios) for i in range(num_external_lcs)] # for storing external lc info
        best_u2_p = [np.zeros(N_scenarios) for i in range(num_external_lcs)] # for storing external lc info
            ...
            if i == 0:
                ...
                if (np.isnan(M_s) or np.isnan(R_s)
                    ...
                    if "PEB" in drop_scenario:
                        ...
                        best_R_comp[j] = res_twin["R_comp"][0]
                        best_fluxratio_EB[j] = res_twin["fluxratio_EB"][0]
                        best_fluxratio_comp[j] = res_twin["fluxratio_comp"][0]
                        lnZ[j] = res_twin["lnZ"]
                        for i in range(num_external_lcs):
        ...
        for i in range(num_external_lcs):
            prob_df[f'flux_ratio_EB_{filt_lcs[i]}_{i+1}'] = best_fluxratio_EB_p[i]
            prob_df[f'flux_ratio_comp_{filt_lcs[i]}_{i+1}'] = best_fluxratio_comp_p[i]
        ...
        if (external_lc_files != None):
            ...
            self.fluxratio_comp_p = best_fluxratio_comp_p
    ...
    def plot_fits(self, time: np.ndarray, flux_0: np.ndarray,
                  flux_err_0: float, x_range: list = [],
                  y_range: list = [], nrows: int = 0,
                  save: bool = False,
                  fname: str = None):
        ...
        f, ax = plt.subplots(
            nrows, 3, figsize=(12, len(df)//3*4), sharex=False # change to True
            )
        for i in range(nrows):
            for j in range(3):
                ...
                ax[i, j].errorbar(
                    time, flux, flux_err, fmt="o",
                    color="dodgerblue", elinewidth=1., capsize=0,
    ...
    def plot_fits_palomar(self, x_range: list = [],
        ...
        #palomar_plot_lc = lightkurve.LightCurve(time = time_p, flux = flux_p, flux_err = flux_err_p)
        #palomar_bin_lc = palomar_plot_lc.bin(time_bin_size=3/1440)

        #time = palomar_bin_lc.time.value
        #flux_0 = palomar_bin_lc.flux
        #p_bin_flux_err = palomar_bin_lc.flux_err
        #flux_err_0 = np.mean(p_bin_flux_err)
