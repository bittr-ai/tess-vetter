...
- `build_report(...)` in `tess_vetter.report`
- `render_html(...)` in `tess_vetter.report`

Most callers should import from `tess_vetter.report` (not private files).
...
from tess_vetter.report import build_report, render_html
...
```python
from tess_vetter.api.generate_report import generate_report, EnrichmentConfig
