...
def _pipeline_version() -> str:
    try:
        return metadata.version("tess-vetter")
    except Exception:
        return "unknown"
    ...
    *,
    ...
    if n_in_lc <= 0:
        ...
        if getattr(config, "enable_t0_refine", False):
            try:
                from tess_vetter.validation.ephemeris_refinement import (
        ...
        if tpf_no_transit_coverage:
            ...
            try:
                ...
                if getattr(tpf_stamp, "wcs", None) is not None:
                    try:
                        from tess_vetter.api.wcs_utils import compute_pixel_scale
...
def enrich_worklist(
    ...
    *,
    ...
    """
    from tess_vetter.api.jsonl import append_jsonl, stream_existing_candidate_keys
