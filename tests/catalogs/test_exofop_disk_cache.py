...
from bittr_tess_vetter.catalogs.exofop_target_page import fetch_exofop_target_summary
...
def test_exofop_toi_table_uses_disk_cache(tmp_path: Path) -> None:
    ...
    with patch("bittr_tess_vetter.catalogs.exofop_toi_table.requests.get") as get:
...
def test_exofop_target_page_uses_disk_cache(tmp_path: Path) -> None:
    ...
    with patch("bittr_tess_vetter.catalogs.exofop_target_page.requests.get") as get:
        get.side_effect = RuntimeError("network should not be called")
