...
class _FakeMASTClientTPFFallback:
    """Simulate LC sectors that don't have matching TPF sectors.

    This mirrors real cases where light curves are available in some sectors but only a different
    sector has a cached/available TPF, which previously caused missing pixel feature families.
    """

    def search_lightcurve(self, tic_id: int) -> list[btv_io.SearchResult]:
        return [
            btv_io.SearchResult(tic_id=tic_id, sector=2, author="SPOC", exptime=120.0),
            btv_io.SearchResult(tic_id=tic_id, sector=3, author="SPOC", exptime=120.0),
        ]
    ...
    def download_lightcurve(
        ...
        # Stable flux; exact sector boundaries aren't critical for this test.
        time = np.linspace(90.0, 120.0, 2000, dtype=np.float64)
        flux = np.ones_like(time)
        flux_err = np.ones_like(time) * 5e-4
        quality = np.zeros_like(time, dtype=np.int32)
    ...
    def search_tpf(self, tic_id: int) -> list[btv_io.SearchResult]:
        # Only sector 1 has a TPF.
        return [btv_io.SearchResult(tic_id=tic_id, sector=1, author="SPOC", exptime=120.0)]

    def download_tpf(self, tic_id: int, sector: int, exptime: float = 120.0):
        if int(sector) != 1:
            raise btv_io.LightCurveNotFoundError(f"No TPF for sector {sector}")
        # Build a tiny stamp where one pixel contains a transit-like dip.
        n = 1000
        time = np.linspace(99.5, 100.5, n, dtype=np.float64)
        flux = np.ones((n, 5, 5), dtype=np.float64)
...
def test_pipeline_populates_localization_and_ghost(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert row["pixel_timeseries_verdict"] is not None
    assert row["pixel_timeseries_delta_chi2"] is not None


def test_pipeline_tpf_fallback_populates_pixel_timeseries(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(btv_io, "MASTClient", lambda: _FakeMASTClientTPFFallback())  # type: ignore[assignment]
    ...
    assert row["pixel_timeseries_verdict"] is not None
    assert row["pixel_timeseries_delta_chi2"] is not None
    assert raw["provenance"]["tpf_sector_used"] == 1
    assert any(a.get("fallback") is True for a in raw["provenance"]["tpf_attempts"])
